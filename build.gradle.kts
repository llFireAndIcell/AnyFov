...
//    if (project.platform.isForge) forge { mixinConfig("mixins.${mod_id}.json") }
...
tasks.processResources {
    ...
    val java =
        if (project.platform.mcMinor >= 18) 17
        else if (project.platform.mcMinor == 17) 16
        else 8
...
tasks {
    withType(Jar::class.java) {
        if (project.platform.isFabric) exclude("mcmod.info", "mods.toml")
        else {
            exclude("fabric.mod.json")
            if (project.platform.isLegacyForge) exclude("mods.toml")
            else exclude("mcmod.info")
        }
    }
